---
title: "Countries"
type: "report"
group: "Geographic"
show_in_menu: true
add_on: "Free"
status: "Done"
interactions:
  - "Date Picker"
  - "Advanced Filters"
widgets:
  - row: 1
    columns: ["countries-table"]
---

# Countries

Shows all countries with visitor metrics, engagement data, and traffic distribution in a data table format.

## Report Configuration

- **Type**: Report Page
- **Menu Group**: Geographic
- **Show in Menu**: Yes
- **Add-on**: Free
- **Status**: Done
- **Figma Design**: [Add link when available]

## Available Interactions

- **Date Picker**: Select time period for data
- **[Advanced Filters](../../global/advanced-filters.md)**: Visitor Analysis Filters group

## Widget Layout

### Row 1 (Full Width)

- [Countries Table](../../widgets/countries-table.md)

## Related Documentation

- [Single Country Report](single-country.md)
- [Cities Report](cities.md)
- [Top Countries Widget](../../widgets/top-countries.md)
- [Countries Table Widget](../../widgets/countries-table.md)
- [Global Visitor Distribution Widget](../../widgets/global-visitor-distribution.md)

---

*Last Updated: 2025-12-14*
